...
Whiteout Survival State Alliance Tracker — small PHP app (no framework). Production uses **MySQL** (`config.json` → `db`); falls back to JSON files if DB is unreachable (local).
